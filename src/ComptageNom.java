...
public class ComptageNom {
  static class ComptageNomMapper extends Mapper <LongWritable, Text, Text, IntWritable> {
	...
	String nom = tokenizer.nextToken();
       	contexte.write (new Text(nom), new IntWritable (1)) ;
  }
  }

  public static class ComptageNomReducer extends Reducer <Text, IntWritable, Text, IntWritable> {
  ...
  public static void main (String args []) throws Exception {
    if (args.length != 2) {
      ...
      System.err.println ("Usage : ComptageNom <source> <destination>") ;
      System.exit (-1) ;
    }

    Job job = new Job () ;
    job.setJarByClass (ComptageNom.class) ;
    ...
    job.setMapperClass (ComptageNomMapper.class) ;
    //job.setCombinerClass (ComptageMotCombiner.class) ;
    job.setReducerClass (ComptageNomReducer.class) ;
