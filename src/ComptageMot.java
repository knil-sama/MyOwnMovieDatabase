...
public class ComptageMot {
  static class ComptageMotMapper extends Mapper <LongWritable, Text, Text, IntWritable> {
	...
	while (tokenizer.hasMoreTokens ()) {
        	contexte.write (new Text (tokenizer.nextToken ()), new IntWritable (1)) ;
	}
  }
  }

  public static class ComptageMotReducer extends Reducer <Text, IntWritable, Text, IntWritable> {
  ...
  public static void main (String args []) throws Exception {
    if (args.length != 2) {
      ...
      System.err.println ("Usage : ComptageMot <source> <destination>") ;
      System.exit (-1) ;
    }

    Job job = new Job () ;
    job.setJarByClass (ComptageMot.class) ;
    ...
    job.setMapperClass (ComptageMotMapper.class) ;
    //job.setCombinerClass (ComptageMotCombiner.class) ;
    job.setReducerClass (ComptageMotReducer.class) ;
