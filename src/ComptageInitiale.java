...
public class ComptageInitiale {
  static class ComptageInitialeMapper extends Mapper <LongWritable, Text, Text, IntWritable> {
	...
	while (tokenizer.hasMoreTokens ()) {
        	contexte.write (new Text (tokenizer.nextToken ().charAt(0)+""), new IntWritable (1)) ;
	}
  }
  }

  public static class ComptageInitialeReducer extends Reducer <Text, IntWritable, Text, IntWritable> {
  ...
  public static void main (String args []) throws Exception {
    if (args.length != 2) {
      ...
      System.err.println ("Usage : ComptageInitiale <source> <destination>") ;
      System.exit (-1) ;
    }

    Job job = new Job () ;
    job.setJarByClass (ComptageInitiale.class) ;
    ...
    job.setMapperClass (ComptageInitialeMapper.class) ;
    //job.setCombinerClass (ComptageMotCombiner.class) ;
    job.setReducerClass (ComptageInitialeReducer.class) ;
